class Vector:
    def __init__(self, values):
        ...
        if type(values) is int:
            ...
            assert len(values) is 2 and values[0] < values[1] and \
                    all(isinstance(x, int) for x in values),\
    ...
    def sum(self, right):
        new_list = [float(self.values[i] + right.values[i])
    ...
    def sub(self, left, right):
        new_list = [float(left.values[i] - right.values[i])
                    for i in range(0, left.size)]
        return Vector(new_list)

    def __truediv__(self, other):
        assert isinstance(other, int) and other != 0,\
            "must be an in different of 0"
        right = self.transform(other)
        return self.div(self, right)

    def __rtruediv__(self, other):
        assert isinstance(self, int) and other != 0,\
            "must be an in different of 0"
        left = self.transform(other)
    ...
    def mul(self, other):
        new_list = [float(self.values[i] * other.values[i])
                    for i in range(0, self.size)]
        return Vector(new_list)
