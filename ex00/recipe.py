class Recipe:
    def __init__(self, name, cooking_lvl, cooking_time, ingredients,
        ...
        assert name and isinstance(name, str), \
            "name id missing or name isn't a str"
        assert cooking_lvl and isinstance(cooking_lvl, int) \
            and 5 >= cooking_lvl >= 1, \
            "cooking should be an integer and between 1 and 5 "
        assert cooking_time and isinstance(cooking_time, int) and \
            (cooking_time > 0), "cooking time should be a positive integer"
        ...
        assert recipe_type and isinstance(recipe_type, str) \
            and (recipe_type == "starter" or recipe_type == "lunch" or
                 recipe_type == "dessert"), \
    ...
    def __str__(self):
        ...
        formatted_string += "\n\tdesc\t\t: {4}\n\trecipe type\t\t: {5}\n"
        return formatted_string.format(self.name.upper(), self.cooking_lvl,
